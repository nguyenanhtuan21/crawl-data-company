...
def get_pdf_file(taxcode):
    try:
        ...
        logging.info(f"Starting get_pdf_file for {taxcode}")
        # URL của trang web
        ...
        response_post = session.post(url, data=payload, verify=False, timeout=7)
        ...
        response_post_download = session.post(url, data=urllib.parse.urlencode(payload_temp), headers=header2, verify=False, timeout=7)
        ...
        if response_post_download.status_code == 200:
            ...
            logging.info(f"PDF file saved successfully for {taxcode}")
        else:
